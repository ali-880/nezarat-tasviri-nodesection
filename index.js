...
const Connect=async()=>{
    await mongoose.connect('mongodb+srv://11401140ali:11401140ali@skuchat.gavz8.mongodb.net/rayan?retryWrites=true&w=majority',{useNewUrlParser:true,useUnifiedTopology:true},(e)=>{
        if(e){
            console.log('first')
            console.log(e)
        }else{
...
mySocket.on('connection',(socket)=>{
    ...
    socket.on('joinTeacherCahtWithStudent',(data)=>{
        console.log('kamal join')
        socket.join(`${data.exam}`)
    })
    ...
    socket.on('handleSendMessageForTeacher',async(data)=>{
        ...
        mySocket.to(`${data.lesson}`).emit('handleGetTeacherMessageFromStudent',course.chatTeacher[index].msg[d-1])
    })
socket.on('handleSendMessageForTeacherExamSection',async(data)=>{
    const course=await Course.findOne({_id:data.lesson}).select('chatTeacher')
    ...
    await course.save();
    mySocket.to(`${data.student} ${data.lesson}`).emit('handleSendMessageForTeacher',course.chatTeacher[index].msg[d-1])
    mySocket.to(`${data.exam}`).emit('handleGetTeacherMessageFromStudent',course.chatTeacher[index].msg[d-1])
    mySocket.to(`${data.exam}`).emit('musicForSms',course.chatTeacher[index].msg[d-1].sender)
})
